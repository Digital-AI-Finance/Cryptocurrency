...
\begin{itemize}
\item Lesson 1 concepts (blocks, hashing, blockchain basics)
\item Basic mathematics (multiplication, exponents)
\item The concept of public and private information
\end{itemize}

\vspace{1em}
\textbf{Software needed:}
\begin{itemize}
\item Python 3.7 or later (for cryptography demonstrations)
\item \texttt{ecdsa} library: \texttt{pip install ecdsa}
\item Text editor or IDE (optional)
\end{itemize}

\vspace{1em}
\textbf{What you'll gain:}\\
Understanding how cryptography secures blockchain transactions and wallets.

\bottomnote{This lesson builds directly on Lesson 1's blockchain fundamentals.}
\end{frame}
...
Your \textbf{private key is like a house key} -- only you should have it. It unlocks your wallet and lets you spend funds. Lose it, and you're locked out forever. Share it, and someone else can take everything!
\end{block}
...
\vspace{0.3em}
\begin{block}{Real-World Analogy}
Your \textbf{public key is like your home address} -- you give it to anyone who needs to send you something. It's safe to share because it doesn't let anyone into your house!
\end{block}
\end{columns}
...
\vspace{0.3em}
\begin{block}{Real-World Analogy}
A digital signature is like \textbf{signing with invisible ink that only shows under UV light}. Everyone can verify it's YOUR unique signature, but no one can copy it or forge it. Each message gets its own unique signature!
\end{block}
...
\begin{itemize}
\item \checkmark Deterministic, one-way, collision-resistant
\item \checkmark SHA-256 is the blockchain standard
\item \checkmark Avalanche effect ensures security
\item \checkmark Used for blocks, Merkle trees, and PoW
\end{itemize}

\vspace{0.5em}
\textbf{Public Key Cryptography}
\begin{itemize}
\item \checkmark Private key: secret, used to sign
\item \checkmark Public key: shareable, used to verify
\item \checkmark ECC (SECP256k1) used in blockchain
\item \checkmark Enables trustless ownership
\end{itemize}

\column{0.48\textwidth}
\textbf{Digital Signatures}
\begin{itemize}
\item \checkmark ECDSA signs transactions
\item \checkmark Proves ownership without revealing key
\item \checkmark Non-repudiation: cannot deny signing
\item \checkmark Foundation of blockchain security
\end{itemize}

\vspace{0.5em}
\textbf{Wallets}
\begin{itemize}
\item \checkmark Key management, not coin storage
\item \checkmark Seed phrase generates unlimited addresses
\item \checkmark Hot wallets: convenient, less secure
\item \checkmark Cold wallets: secure, less convenient
\end{itemize}
\end{columns}

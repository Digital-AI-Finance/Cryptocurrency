...
\begin{itemize}
\item ERC-20 defines fungible tokens
\item Standards enable interoperability
\item 6 functions + 2 events
\end{itemize}

\item \textbf{Core Functions}
\begin{itemize}
\item transfer() for direct sends
\item approve()/transferFrom() for delegation
\item Events for transparency
\end{itemize}

\item \textbf{Development}
\begin{itemize}
\item OpenZeppelin for security
\item Hardhat for tooling
\item Test before deploy
\end{itemize}
\end{enumerate}
...
\item[\textcolor{mlgreen}{$\checkmark$}] Standards enable ecosystem
\item[\textcolor{mlgreen}{$\checkmark$}] Inheritance simplifies code
\item[\textcolor{mlgreen}{$\checkmark$}] Tokenomics matter
...
\begin{itemize}
\item Read and understand ERC-20
\item Create custom tokens
\item Deploy with Hardhat
\item Design tokenomics
\item Evaluate token projects
\end{itemize}
\end{columns}

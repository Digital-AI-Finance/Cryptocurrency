...
\begin{itemize}
\item Ethereum = programmable blockchain
\item EVM executes smart contracts
\item Two account types: EOA and Contract
\item State stored on-chain globally
\end{itemize}

\vspace{0.5em}
\textbf{Gas Mechanics}
\begin{itemize}
\item Gas measures computation cost
\item Prevents infinite loops
\item Fee = Gas Used $\times$ Gas Price
\item Storage operations are expensive
\end{itemize}

\column{0.48\textwidth}
\textbf{Solidity Essentials}
\begin{itemize}
\item State variables = persistent storage
\item Functions: public, external, view, pure
\item Events for logging (cheaper than storage)
\item Modifiers for access control
\end{itemize}

\vspace{0.5em}
\textbf{Development Workflow}
\begin{itemize}
\item Write $\rightarrow$ Compile $\rightarrow$ Deploy $\rightarrow$ Interact
\item Use Hardhat for local development
\item Test thoroughly before mainnet
\item Call (free) vs Transaction (gas)
\end{itemize}
\end{columns}

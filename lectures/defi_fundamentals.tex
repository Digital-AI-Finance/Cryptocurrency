...
\begin{frame}{DeFi Ecosystem Map}
  ...
  \begin{tikzpicture}[
    ...
    % Stablecoins
    \node[cat, fill=mlorange!20]  (stbl) at (0.0, 3.2) {Stablecoins};
    \node[sub]                    (mkr)  at (0.0, 2.4) {MakerDAO};
    ...
    % Aggregators
    \node[cat, fill=mllavender!60](agg)  at (0.0,-3.2) {Aggregators};
    \node[sub]                    (onei) at (0.0,-4.0) {1inch};
...
\begin{frame}{Liquidation Mechanics}
  ...
  \begin{tikzpicture}[
    ...
    \node[stepbox, fill=mlorange!25] (s4) at (10.0,-2.2) {4. Liquidator repays\\{\footnotesize up to 50\% of debt + bonus}};
    % Step 5: position reduced
    \node[stepbox, fill=mlblue!20]   (s5) at (5.0, -2.2) {5. Collateral seized\\{\footnotesize position partially closed}};
    % Step 6: protocol solvent
    \node[stepbox, fill=mlgreen!25]  (s6) at (0,   -2.2) {6. Protocol remains\\{\footnotesize solvent, HF restored}};
    ...
    \node[rectangle, fill=mlorange!25, draw=mlorange, rounded corners=2pt,
      ...
      at (5.0, -3.6) {$1.0 < $ HF $ < 1.5$: Warning};
    \node[rectangle, fill=mlred!20,    draw=mlred,    rounded corners=2pt,
          minimum width=2.0cm, minimum height=0.45cm, align=center, font=\tiny\bfseries]
      at (8.4, -3.6) {HF $< 1.0$: Liquidation};
  \end{tikzpicture}
...
\begin{frame}{Compound vs.\ Aave: Protocol Comparison}
  ...
  \begin{tikzpicture}[
    ...
    \node[rectangle, fill=mllavender4, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -0.8) {Governance token};
    \node[celA, minimum width=4.4cm] at (4.1,  -0.8) {COMP};
    \node[celB, minimum width=4.4cm] at (8.5,  -0.8) {AAVE};

    % Row 2: Interest rate model
    \node[rectangle, fill=mllavender3, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -1.6) {Rate model};
    \node[altA, minimum width=4.4cm] at (4.1, -1.6) {Kinked (per market)};
    \node[altB, minimum width=4.4cm] at (8.5, -1.6) {Kinked + stable rate};

    % Row 3: Multi-chain
    \node[rectangle, fill=mllavender4, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -2.4) {Multi-chain};
    \node[celA, minimum width=4.4cm] at (4.1,  -2.4) {Ethereum + limited};
    \node[celB, minimum width=4.4cm] at (8.5,  -2.4) {8+ chains (Portal)};

    % Row 4: Flash loans
    \node[rectangle, fill=mllavender3, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -3.2) {Flash loans};
    \node[altA, minimum width=4.4cm] at (4.1, -3.2) {No (native)};
    \node[altB, minimum width=4.4cm] at (8.5, -3.2) {Yes (0.09\% fee)};

    % Row 5: Token standard
    \node[rectangle, fill=mllavender4, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -4.0) {Yield token};
    \node[celA, minimum width=4.4cm] at (4.1,  -4.0) {cToken (exchange rate)};
    \node[celB, minimum width=4.4cm] at (8.5,  -4.0) {aToken (rebasing)};

    % Row 6: Liquidation
    \node[rectangle, fill=mllavender3, minimum height=0.72cm, minimum width=3.8cm,
          align=center, font=\scriptsize] at (0,   -4.8) {Liquidation cap};
    \node[altA, minimum width=4.4cm] at (4.1, -4.8) {50\% of position};
    \node[altB, minimum width=4.4cm] at (8.5, -4.8) {50\% (default)};
  \end{tikzpicture}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        % CDP flow
        \node[procbox, fill=mlblue!20]   (dep)  at (0, 4.8) {1. Deposit ETH\\into Vault (CDP)};
        \node[procbox, fill=mlgreen!20]  (mint) at (0, 3.4) {2. Mint DAI\\(up to 66\% of value)};
        \node[procbox, fill=mlorange!20] (use)  at (0, 2.0) {3. Use DAI\\in DeFi ecosystem};
        \node[procbox, fill=mlpurple!15] (fee)  at (0, 0.6) {4. Accrue Stability Fee\\(annual DAI interest)};
        \node[procbox, fill=mlgreen!15]  (rep)  at (0,-0.8) {5. Repay DAI + fee\\to reclaim ETH};
        ...
        % Liquidation branch
        \node[procbox, fill=mlred!20] (liq) at (4.8, 0.6) {Liquidation\\triggered at 150\%};
        \draw[-{Stealth[length=6pt]}, thick, mlred, dashed] (fee.east) -- (liq.west)
          node[midway, above, font=\tiny\bfseries, mlred] {price drops};

        % Peg mechanism
        \node[font=\scriptsize, mlgray, align=center] at (0,-2.0)
...
\begin{frame}{Section 4 Summary}
  \vspace{2mm}
  \begin{tikzpicture}[
    sumbox/.style={rectangle, rounded corners=5pt, minimum width=13.5cm, minimum height=0.85cm,
    ...
    \node[sumbox, fill=mlpurple!15]  at (0,-1.0) {\textbf{APY vs APR}: Daily compounding at 50\% APR yields 64.8\% APY; advertised figures are often APY};
    \node[sumbox, fill=mlorange!20]  at (0,-2.0) {\textbf{Impermanent Loss}: $\text{IL}(r)=\frac{2\sqrt{r}}{1+r}-1$; 2$\times$ price move costs 5.7\%; grows nonlinearly};
    \node[sumbox, fill=mlgreen!20]   at (0,-3.0) {\textbf{Strategies}: spectrum from single-sided staking (safe) to leveraged farming (highest risk)};
    \node[sumbox, fill=mlred!15]     at (0,-4.0) {\textbf{Real Yield}: always distinguish fee revenue from inflationary token emissions};
  \end{tikzpicture}
  \vspace{3mm}
  \begin{alertblock}{Key Insight}
...
\begin{frame}{Key Takeaways and Course Summary}
  \vspace{1mm}
  \begin{tikzpicture}[
    tkbox/.style={rectangle, rounded corners=5pt, minimum width=13.5cm, minimum height=0.85cm,
      draw=mlpurple, thick, align=center, font=\small\bfseries}
  ]

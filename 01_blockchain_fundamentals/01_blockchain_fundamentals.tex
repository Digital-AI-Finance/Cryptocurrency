...
\begin{itemize}
\item Basic computer literacy (how computers and networks work)
\item Interest in finance and/or technology
\item Curiosity about how digital systems work
\end{itemize}

\vspace{1em}
\textbf{Software needed:}
\begin{itemize}
\item Python 3.7 or later (for code demonstrations)
\item A web browser (to view blockchain explorers)
\item Text editor or IDE (optional, for experimenting with code)
\end{itemize}

\vspace{1em}
\textbf{No prior blockchain knowledge required!}

\bottomnote{This is Lesson 1 -- we start from the very beginning.}
\end{frame}
...
Think of blockchain like a \textbf{chain of linked train cars}. Each car (block) contains cargo (data) and is connected to the previous car. You can't remove or change a car in the middle without breaking the entire chain!
\end{block}
...
\vspace{0.5em}
\begin{block}{Real-World Analogy}
A block hash is like a \textbf{fingerprint} -- unique to that specific block. Change even one tiny detail in the block, and you get a completely different ``fingerprint''!
\end{block}
...
\vspace{0.3em}
\begin{block}{Real-World Analogy}
Proof of Work is like finding a \textbf{winning lottery ticket}. You have to try millions of random numbers (nonces) until you find one that produces a hash starting with enough zeros. It's hard to find, but easy for everyone else to verify you won!
\end{block}
...
\begin{itemize}
\item \checkmark \textbf{Distributed ledger:} No central authority needed
\item \checkmark \textbf{Immutability:} Cryptographic linking prevents tampering
\item \checkmark \textbf{Consensus:} Network agrees on one version of truth
\item \checkmark \textbf{Transparency:} Anyone can verify transactions
\end{itemize}

\vspace{0.5em}
\textbf{Block Structure}
\begin{itemize}
\item \checkmark Header contains metadata (hash, prev\_hash, nonce)
\item \checkmark Body contains transaction data
\item \checkmark Merkle root enables efficient verification
\end{itemize}

\column{0.48\textwidth}
\textbf{Consensus Mechanisms}
\begin{itemize}
\item \checkmark \textbf{PoW:} Computational work proves legitimacy
\item \checkmark \textbf{PoS:} Economic stake proves commitment
\item \checkmark Both solve the double-spending problem
\end{itemize}
